...
public class Timer {
	...
	public void checkIncrement() {
		...
		// pr�fen des T0CS bit (externer Takt �ber Ra4 == 0)
		if (ctr.getMemo().getBitValue(129, 5) == 1) {
			// Pr�fen ob T0Se bit gesetzt == fallende Flanke oder nicht == steigenden Flanke
			if (ctr.getMemo().getBitValue(129, 4) == 0) {
				// pr�fen on Steigende Flanke anliegt, diese kann von 0 zu 1 aber auch von 1 zu
				// 0gehen
				if (raEdge == 1) {
	...
	// Getter f�r Vorteiler
	public double getVorteiler() {
		return Vorteiler;
	}

	// Setter f�r Vorteiler
